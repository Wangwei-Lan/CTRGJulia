...
function ConstructProjectorCenter2(projcenter1::Array{Float64},chimax::Int64)  
    # seems best for now
    #pcL = ConstructProjectorCenter4((projcenter1+permutedims(projcenter1,[3,4,1,2]))/2,chimax)
    projcenter = (projcenter1+permutedims(projcenter1,[3,4,1,2]))*0.5
    Dlink = size(projcenter,1);chicl3 = size(projcenter,2)
    projcenter = reshape(projcenter,Dlink*chicl3,Dlink*chicl3)
    @time F = svd(projcenter)
    pcL = F.U
    if Dlink*chicl3> chimax
        pcL = reshape(pcL[:,1:chimax],Dlink,chicl3,chimax)
    else
        pcL = reshape(pcL,Dlink,chicl3,Dlink*chicl3)
    end
    pcR = pcL
    return pcL,pcR
end
...
function  Mover(RGTensor::Array{Array{Float64}},TL::Array{Float64},TR::Array{Float64},chimax::Int64;direction=nothing)
    ...
    if direction == "LD"
        ...
        @time @tensor projcenter1[-1,-2,-3,-4] := cl[4,3,-2]*cl[4,5,-4]*TL[6,7,-1]*TL[8,7,-3]*t2[1,2,6,3]*t2[1,2,8,5]
        #@tensor projcenter2[-1,-2,-3,-4] := cr[5,4,-4]*cr[3,4,-2]*TR[7,8,-3]*TR[7,6,-1]*t1[5,8,2,1]*t1[3,6,2,1]
        #
        @time @tensor projhorizontal[-1,-2,-3,-4] := cl[7,10,6]*cr[3,4,6]*cl[7,9,8]*cr[5,4,8]*T[15,-2,16,14]*
                            T[13,-4,16,12]*t2[11,-1,15,10]*t2[11,-3,13,9]*t1[3,14,2,1]*t1[5,12,2,1]
        @time @tensor projvertical[-1,-2,-3,-4] := cl[4,5,8]*cr[9,7,8]*cl[4,3,6]*cr[10,7,6]*T[12,16,-4,13]*T[14,16,-2,15]*
    ...
    #PcL,PcR = ConstructProjectorCenter(projcenter,chimax)
    #PcR = PcL
    #PcL,PcR = ConstructProjectorCenter1(projcenter,chimax)
    #PcL,PcR = ConstructProjectorCenter2(projcenter1,projcenter2,chimax)
    PcL,PcR = ConstructProjectorCenter2(projcenter1,chimax)
    #
    projhorizontal = reshape(projhorizontal,chicl1*dlink,chicl1*dlink)
    @time F = svd((projhorizontal+projhorizontal')/2)
    ...
    @time F = svd((projvertical+projvertical')/2)
