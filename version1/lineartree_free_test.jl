...
function HMover(T::Array{Float64},A::Array{Float64},ht::Array{Float64},at::Array{Float64},chimax;Asz=nothing)
    ...
    #=
    temp = rand(chiTup,chiAup)
    for j in 1:10
        @tensor temp[-1,-2] := temp[1,2]*T[1,4,-1,3]*A[2,3,-2,4]
        temp = temp/maximum(temp)
    end
    @tensor TEMP[-1,-2,-3,-4] := temp[-1,-2]*temp[-3,-4]
    =#
    @tensor TEMP[-1,-2,-3,-4] := T[-1,1,2,5]*T[-3,1,2,6]*A[-2,5,3,4]*A[-4,6,3,4]
    #@tensor TEMP[-1,-2,-3,-4] := ht[-1,1,2,5]*ht[-3,1,2,6]*at[-2,5,3,4]*at[-4,6,3,4]
    #
    #=
...
function VMover(T::Array{Float64},A::Array{Float64},vt::Array{Float64},at::Array{Float64},chimax::Int64;Asz = nothing)
    A = permutedims(A,[2,3,4,1])
    T = permutedims(T,[2,3,4,1])
    vt = permutedims(vt,[2,3,4,1])
    at = permutedims(at,[2,3,4,1])

    if Asz == nothing
        A,PV = HMover(T,A,vt,at,chimax)
        return permutedims(A,[4,1,2,3]),PV
    else
        Asz = permutedims(Asz,[2,3,4,1])
        A,Asz,PV = HMover(T,A,vt,at,chimax,Asz=Asz)
...
function CorseGraining(T::Array{Float64},NumLayer::Int64;Sz=Float64[1.0 0.0; 0.0 -1.0])
    ...
    for j in 1:NumLayer
        ...
        HT = HT/maximum(HT)
        #------ Left Mover
        CenterDenominator,CenterNumerator,PL = HMover(HT,CenterDenominator,T,VT,chimax,Asz=CenterNumerator)
        @tensor VT[-1,-2,-3,-4] := VT[1,3,5,-4]*T[2,-2,4,3]*PL[2,1,-1]*PL[4,5,-3]

        #------ Right Mover
        CenterDenominator,CenterNumerator,PR = HMover(CenterDenominator,HT,VT,T,chimax,Asz=CenterNumerator)
        ...
        #------ Up Mover
        CenterDenominator,CenterNumerator,PD = VMover(VT,CenterDenominator,T,HT,chimax,Asz=CenterNumerator)
        @tensor HT[-1,-2,-3,-4] := HT[-1,1,3,4]*T[3,2,-3,5]*PD[2,1,-2]*PD[5,4,-4]

        #------ Down Mover
        CenterDenominator,CenterNumerator,PU = VMover(CenterDenominator,VT,HT,T,chimax,Asz=CenterNumerator)
        ...
        FE = (log(Z)+sum(log.(Znorm))+2*sum([1:1:j...].*log.(HTnorm))+
                        2*sum([1:1:j...].*log.(VTnorm)))/Ns
        append!(FEttrg,[FE])
        append!(NumSite,[Ns])
    end
...
#for T in 0.3:0.05:0.3
    Beta = 0.3#0.4406#1/Temperature
    #Temperature = 1.0
    ...
    #for NumLayer in 100:100:100
        ...
        rhoDenominator,rhoNumerator,HT,VT,Znorm,HTnorm,VTnorm,FEttrg,NumSite = CorseGraining(T,NumLayer,Sz=sx)
